...
maze = []
for i in range(row):
    converted = [int(c) for c in sys.stdin.readline().rstrip()]
    maze.append(list(converted))
...
def bfs(graph, start):
    ...
    while q:
        ...
        for i in range(4):
            ...
            elif graph[n_row][n_col] == 1:
                graph[n_row][n_col] = graph[c_row][c_col] + 1
                q.append((n_row, n_col))
            # else:
            #     q.append((n_row, n_col))
            #     graph[n_row][n_col] = min(
            #         graph[c_row][c_col] + 1, graph[n_row][n_col])
