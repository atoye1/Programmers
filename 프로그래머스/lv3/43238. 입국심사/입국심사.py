...
2. 심사하는데 걸리는 최대한의 시간은 customer  * max(times)다.
4. times배열은 10만 이하니까 정렬해도 괜찮다.
5. 잠깐, 이분탐색은 정렬한 것을 대상으로 하는거니깐 times를 대상으로 이분탐색 하는거야?
	아닌것같은데...
6. 결국 time을 대상으로 이분탐색하는게 맞았다.
7. 이 문제도 파라메트릭 서치였던 것이다.. 두둥
'''
...
def solution(n, times):
    ...
    while start <= end:
        ...
        processed = sum([mid // time for time in times])
        # print('mid', mid, 'people', people)
        if processed >= n: # 조건을 만족한 경우이므로 더 작은쪽을 탐색한다.
            answer = mid
            end = mid - 1
